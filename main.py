...
def get_args_parser():
    ...
    parser.add_argument('--lr_pose_net', default=1e-5, type=float)
    ...
    parser.add_argument('--verb_pose_loss_coef', default=2, type=float)
    ...
    parser.add_argument('--pose_only_mode', default=0, type=int)
    parser.add_argument('--train_pose_net', default=0, type=int)
    ...
    parser.add_argument('--cfg',
                        ...
                        default='config/w32_512_adam_lr1e-3.yaml',
                        type=str)
...
def main(args):
    ...
    model, criterion, postprocessors = build_model(args, cfg)
    if args.train_pose_net:
        model.pose_net = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model.pose_net)
    ...
    if args.train_pose_net == 0:
        for name, p in model.named_parameters():
            if 'pose_net' in name:
                p.requires_grad = False

    if args.pose_only_mode == 1:
        for name, p in model.named_parameters():
            if 'pose_t' in name:
                p.requires_grad = True
            else:
    ...
    if args.pose_only_mode == 1:
        param_dicts= [
            {"params": [p for n, p in model_without_ddp.named_parameters() if "pose_t" in n and p.requires_grad]},
        ]
    else:
        param_dicts = [
            {"params": [p for n, p in model_without_ddp.named_parameters() if "backbone" not in n and "pose_net" not in n and p.requires_grad]},
            {
                "params": [p for n, p in model_without_ddp.named_parameters() if "backbone" in n and p.requires_grad],
                "lr": args.lr_backbone,
            },
            {
                "params": [p for n, p in model_without_ddp.named_parameters() if "pose_net" in n and p.requires_grad],
                "lr": args.lr_pose_net,
            },
        ]
    ...
    if args.eval:
        ...
        return
    model_without_ddp.pose_net.load_state_dict(torch.load('params/pose_higher_hrnet_w32_512.pth', map_location='cpu'),
                                               strict=False)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            ...
            args.clip_max_norm, args.train_pose_net, args.eval_loss)
        if args.eval_loss:
            exit()
        lr_scheduler.step()
