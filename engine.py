...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
                    ...
                    device: torch.device, epoch: int, max_norm: float = 0, train_pose_net=False, eval_loss = False):
    model.train()
    if not train_pose_net:
...
def evaluate_hoi(dataset_file, model, postprocessors, data_loader, subject_category_id, device, args):
    ...
    for samples, targets in metric_logger.log_every(data_loader, 10, header):
        ...
        now_target_sizes = torch.stack([t["vsize"] for t in targets], dim=0)

        #target_sizes = torch.stack([(sample.shape(2), sample.shape(3)) for sample in targets], dim=0)

        results = postprocessors['hoi'](outputs, orig_target_sizes, now_target_sizes)
