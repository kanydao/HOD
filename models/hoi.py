...
class CDNHOI(nn.Module):
    ...
    def __init__(self, backbone, pose_net, transformer, num_obj_classes, num_verb_classes, num_queries, aux_loss=False, args=None):
        super().__init__()
        ...
        #self.verb_class_embed_pose_t = nn.Linear(hidden_dim, num_verb_classes)
        ...
        #self.input_proj_pose_train = nn.Conv2d(544, hidden_dim, kernel_size=1)

        self.pose_conv = pose_conv()
        self.pose_feature_conv = pose_feature_conv(32)
    ...
    def forward(self, samples: NestedTensor):
        ...
        pose, pose_feature = self.pose_net(samples.tensors)
        pose = pose[0]

        pose = pose[:, 17:34]
        #pose = torch.cat((pose, pose_feature), dim=1)
        pose = torch.cat((pose[:, 1:3], pose[:, 5:7], pose[:, 9:13], pose[:, 15:17]), dim=1)


        pose_attention = self.pose_conv(pose)
        pose_feature = self.pose_feature_conv(pose_feature)
        #pose_feature = pose_feature * pose_attention + pose_feature


        assert mask is not None
        hopd_out, hopd_out2, interaction_decoder_out, interaction_decoder_out2 = self.transformer(self.input_proj(src), pose_feature, pose_attention, mask, self.query_embed.weight, self.query_embed2.weight,pos[-1])


        outputs_sub_coord = self.sub_bbox_embed(hopd_out2).sigmoid()
        ...
        outputs_verb_class2 = self.verb_class_embed(interaction_decoder_out2)
        outputs_verb_class_pose = outputs_verb_class


        out = {'pred_obj_logits': outputs_obj_class[-1], 'pred_verb_logits': outputs_verb_class[-1],
               'pred_sub_boxes': outputs_sub_coord[-1], 'pred_obj_boxes': outputs_obj_coord[-1],
               'pred_verb_logits2': outputs_verb_class2[-1], 'pred_verb_logits_pose': outputs_verb_class_pose[-1]}
        if self.use_matching:
            out['pred_matching_logits'] = outputs_matching[-1]

        if self.aux_loss:
            if self.use_matching:
                out['aux_outputs'] = self._set_aux_loss2(outputs_obj_class, outputs_verb_class,
                                                        outputs_sub_coord, outputs_obj_coord,
                                                        outputs_verb_class2, outputs_verb_class_pose,
                                                        outputs_matching)
            else:                                            
                out['aux_outputs'] = self._set_aux_loss2(outputs_obj_class, outputs_verb_class,
                                                        outputs_sub_coord, outputs_obj_coord,
    ...
    def _set_aux_loss2(self, outputs_obj_class, outputs_verb_class, outputs_sub_coord, outputs_obj_coord, outputs_verb_class2, outputs_verb_class_pose, outputs_matching=None):
        min_dec_layers_num = min(self.dec_layers_hopd, self.dec_layers_interaction)
        if self.use_matching:
            return [{'pred_obj_logits': a, 'pred_verb_logits': b, 'pred_sub_boxes': c, \
                     'pred_obj_boxes': d, 'pred_matching_logits': e, 'pred_verb_logits2': f, 'pred_verb_logits_pose': g}
                    for a, b, c, d, e, f, g in zip(outputs_obj_class[-min_dec_layers_num : -1], outputs_verb_class[-min_dec_layers_num : -1], \
                                             outputs_sub_coord[-min_dec_layers_num : -1], outputs_obj_coord[-min_dec_layers_num : -1], \
                                             outputs_matching[-min_dec_layers_num : -1], outputs_verb_class2[-min_dec_layers_num : -1],
                                                   outputs_verb_class_pose[-min_dec_layers_num : -1])]
        else:
            return [{'pred_obj_logits': a, 'pred_verb_logits': b, 'pred_sub_boxes': c, 'pred_obj_boxes': d, 'pred_verb_logits2': e,'pred_verb_logits_pose': f}
                    for a, b, c, d, e, f in zip(outputs_obj_class[-min_dec_layers_num : -1], outputs_verb_class[-min_dec_layers_num : -1], \
                                          outputs_sub_coord[-min_dec_layers_num : -1], outputs_obj_coord[-min_dec_layers_num : -1],
                                                outputs_verb_class2[-min_dec_layers_num: -1], outputs_verb_class_pose[-min_dec_layers_num : -1])]
...
class pose_conv(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
                      nn.Conv2d(2, self.dim1, kernel_size=7, stride=2, padding=3, bias=False),
                      nn.ReLU(inplace=True)
                      )
        self.conv2 = nn.Sequential(
                      nn.Conv2d(self.dim1, 1, kernel_size=3, stride=2, padding=1, bias=False),
                      nn.Sigmoid()
                    )
    def forward(self, x):
        y = self.maxpool(x)
        y_max, _ = torch.max(y, dim=1, keepdim=True)
        y = torch.cat((torch.mean(y, dim=1, keepdim=True), y_max), dim=1)
        y = self.conv1(y)
        #out2 = F.interpolate(pose2, scale_factor=0.25,mode="bilinear")
        y = self.conv2(y)
        return y

class pose_feature_conv(nn.Module):
    def __init__(self,inputdim):
        ...
        self.avgpool = nn.AvgPool2d(kernel_size=3, stride=2, padding=1)
        self.conv1 = nn.Sequential(
                      nn.Conv2d(inputdim, self.dim1, kernel_size=3, stride=2, padding=1, bias=False),
                      nn.ReLU(inplace=True)
                      )
        self.conv2 = nn.Sequential(
            nn.Conv2d(self.dim1, self.dim2, kernel_size=3, stride=1, padding=1, bias=False),
            nn.ReLU(inplace=True)
        )
        self.conv3 = nn.Sequential(
                      nn.Conv2d(self.dim2, self.dim3, kernel_size=3, stride=2, padding=1, bias=False)
                    )
    def forward(self, x):
        y = self.maxpool(x)
        y = self.conv1(y)
...
class SetCriterionHOI(nn.Module):
    ...
    def __init__(self, num_obj_classes, num_queries, num_verb_classes, matcher, weight_dict, eos_coef, losses, args):
        ...
        if args.dataset_file == 'hico':
            ...
            self.obj_nums_init = [0, 5397, 238, 332, 321, 5, 6, 45, 90, 59, 20, \
    ...
    def loss_obj_labels(self, outputs, targets, indices, num_interactions, log=True):
        ...
        if not self.obj_reweight:
            ...
            alpha = min(math.pow(0.999, self.q_obj.qsize()), 0.9)
            obj_weights = alpha * self.obj_weights_init + (1 - alpha) * obj_weights
    ...
    def loss_verb_labels(self, outputs, targets, indices, num_interactions):
        ...
        if not self.verb_reweight:
            ...
            alpha = min(math.pow(0.999, self.q_verb.qsize()),0.9)
            verb_weights = alpha * self.verb_weights_init + (1 - alpha) * verb_weights
    ...
    def loss_verb_labels2(self, outputs, targets, indices, num_interactions):
        assert 'pred_verb_logits2' in outputs
        src_logits = outputs['pred_verb_logits2']
        ...
        losses = {'loss_verb2_ce': loss_verb_ce}
        return losses

    def loss_verb_labels_pose(self, outputs, targets, indices, num_interactions):
        assert 'pred_verb_logits_pose' in outputs
        src_logits = outputs['pred_verb_logits_pose']
        ...
        losses = {'loss_verb_ce_pose': loss_verb_ce}
        return losses
    ...
    def get_loss(self, loss, outputs, targets, indices, num, **kwargs):
        loss_map = {
            ...
            'verb_labels2': self.loss_verb_labels2,
            'verb_labels_pose': self.loss_verb_labels_pose,
...
class PostProcessHOI(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, outputs, target_sizes, now_target_sizes):
        out_obj_logits = outputs['pred_obj_logits']
        out_verb_logits = outputs['pred_verb_logits']
        #out_verb_logits = outputs['pred_verb_logits']+outputs['pred_verb_logits_pose']
        #out_verb_logits = torch.minimum(outputs['pred_verb_logits'], outputs['pred_verb_logits_pose'])
        ...
        img_h1, img_w1 = now_target_sizes.unbind(1)
        scale_fct = torch.stack([img_w, img_h, img_w, img_h], dim=1).to(verb_scores.device)
        scale_fct1 = torch.stack([img_w1, img_h1, img_w1, img_h1], dim=1).to(verb_scores.device)
        scale_fct2 = (scale_fct-scale_fct1)/2
        sub_boxes = box_cxcywh_to_xyxy(out_sub_boxes)
        #sub_boxes = sub_boxes * scale_fct[:, None, :]
        sub_boxes = sub_boxes * scale_fct1[:, None, :] + scale_fct2[:, None, :]
        obj_boxes = box_cxcywh_to_xyxy(out_obj_boxes)
        obj_boxes = obj_boxes * scale_fct1[:, None, :] + scale_fct2[:, None, :]
...
def build(args, cfg):
    ...
    pose_net = get_pose_net(cfg, is_train=True)
    #pose_net = 0
    cdn = build_cdn(args)
    ...
    weight_dict['loss_verb_ce'] = args.verb_loss_coef *0.5
    weight_dict['loss_verb2_ce'] = args.verb_loss_coef *0.5
    weight_dict['loss_sub_bbox'] = args.bbox_loss_coef
    weight_dict['loss_obj_bbox'] = args.bbox_loss_coef
    weight_dict['loss_sub_giou'] = args.giou_loss_coef
    weight_dict['loss_obj_giou'] = args.giou_loss_coef
    weight_dict['loss_verb_ce_pose'] = args.verb_pose_loss_coef
    ...
    losses = ['obj_labels', 'verb_labels', 'sub_obj_boxes', 'obj_cardinality', 'verb_labels2']#, 'verb_labels_pose']
    if args.use_matching:
